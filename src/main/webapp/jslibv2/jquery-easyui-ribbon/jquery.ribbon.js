(function($){
	function buildRibbon(target){
		...
		if(opts.headerMargin !== undefined){
			$(target).find(".tabs-header").attr({style: "width:100%;margin-left:" + opts.headerMargin + "px;z-index:200;"});
		}
	}
	...
	function bindEvents(target){
		...
		$(target).find('.l-btn').unbind('.ribbon').bind('click.ribbon', function(e){
			...
			$(target).find('.l-btn').each(function(e){
