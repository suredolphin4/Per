...
var moduleLoaded = function() {
	$('#admin_yhgl_datagrid').datagrid({
		url : $('#mapCateContextPath').val() + '/userAction!datagrid.action',
		fit : true,
		fitColumns : true,
		border : false,
		pagination : true,
...
function append() {
	 $.ajax( {    
		    url:$('#mapCateContextPath').val() + '/roleAction!datagrid.action',// 跳转到 action    
		   /*  data:{    
	...
	if (rows.length > 0) {
		$.messager.confirm('确认', '您是否要删除当前选中的项目？', function(r) {
			if (r) {
				...
				$.ajax({
					url : $('#mapCateContextPath').val() + '/userAction!remove.action',
					data : {
						ids : ids.join(',')
					},
					dataType : 'json',
