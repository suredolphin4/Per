...
@Result(name = "toProject", location = "/performance/modules/project/project.jsp") ,
@Result(name = "toProjectEdit", location = "/performance/jsglEdit.jsp") ,
})
@Namespace("/")
@ParentPackage("basePackage")
public class ProjectAction extends BaseAction<Xm> implements SessionAware,UserAware, ModelDriven<Xm> {

	private  User userObj;
	private Map<String, Object> sessionMap;
	private Xm graduate = new Xm();
	...
	public ProjectServiceI graduateService;
	public ProjectServiceI getProjectService() {
		return graduateService;
	}

	@Autowired
	public void setProjectService(ProjectServiceI graduateService) {
	...
	public void add() {
		...
		try {
			...
			Xm r = graduateService.save(graduate);
			j.setSuccess(true);
			j.setMsg("添加成功！");
			j.setObj(r);
	...
	public void edit(){
		...
		try {
			Xm r = graduateService.edit(graduate);
			j.setSuccess(true);
			j.setMsg("修改成功！");
			j.setObj(r);
		}catch(DataIntegrityViolationException e){
