...
@Result(name = "toReport", location = "/performance/modules/report/report.jsp") ,
@Result(name = "toReportEdit", location = "/performace/jsglEdit.jsp") ,
})
@Namespace("/")
@ParentPackage("basePackage")
public class ReportAction  extends BaseAction<Zx> implements SessionAware,UserAware, ModelDriven<Zx> {

	private  User userObj;
	private Map<String, Object> sessionMap;
	private Zx zx = new Zx();
	...
	public ReportServiceI reportService;

	public ReportServiceI getReportService() {
	...
	public void add() {
		...
		try {
			//设置新增论文记录时的时间
			zx.setSavetime(new Timestamp(new java.util.Date().getTime()));
			Zx r = reportService.save(zx);
			j.setSuccess(true);
			j.setMsg("添加成功！");
			j.setObj(r);
	...
	public void remove() {
		...
		try{
	        ...
	        String ids = jsonobj.getString("zxids");
	...
	public void datagrid() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if("秘书".equals(role.getRolename())){
				...
				//user list
				if(usernames.size()!=0){
					dataGrid=reportService.datagrid(zx, "", usernames);				
				}
				
			}else if("管理员".equals(role.getRolename())){
				dataGrid=reportService.datagrid(zx, "",null);
				
			}else{
				dataGrid=reportService.datagrid(zx, getUserCode(),null);
	...
	public void submit(){
		...
		try{			
			ActionContext context=ActionContext.getContext();     
		    Map  parameterMap=context.getParameters();  
			String[] params = (String[]) parameterMap.get("zxlist");  //接收zxlist参数
			String json = new String(params[0]);
	        JSONObject jsonobj = JSONObject.fromObject(json);
	        JSONArray jsonArray = jsonobj.getJSONArray("zxList");     //json数组对应的键
	        String submitUser = jsonobj.getString("submitUser");
	        List<Zx> zxList = JSONArray.toList(jsonArray, new Zx(), new JsonConfig());
	             
	        //持久化到数据库
	        reportService.submit(zxList, submitUser);
	...
	public void revoke(){
		...
		try{			
			ActionContext context = ActionContext.getContext();     
		    Map  parameterMap = context.getParameters();  
			String[] params = (String[]) parameterMap.get("reportlist");  //接收zxlist参数
			String json = new String(params[0]);
	        JSONObject jsonobj = JSONObject.fromObject(json);
	        JSONArray jsonArray = jsonobj.getJSONArray("reportList");     //json数组对应的键
	        List<Zx> zxList = JSONArray.toList(jsonArray, new Zx(), new JsonConfig());
	             
	        //持久化到数据库
	        reportService.revoke(zxList);
	...
	public void audit(){
		...
		try{
	        ...
	        reportService.audit(auditStatus, auditOpinion, zxList);
	...
	public void downloadExcel() throws Exception {
		...
		String fileName = reportService.exportExcel();
