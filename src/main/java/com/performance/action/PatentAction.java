...
@Action(value = "patentAction", results = { 
@Result(name = "toPatent", location = "/performance/modules/patent/patent.jsp") ,
@Result(name = "toPatentEdit", location = "/performance/jsglEdit.jsp") ,
})
@Namespace("/")
@ParentPackage("basePackage")
public class PatentAction extends BaseAction<Zl> implements SessionAware,UserAware, ModelDriven<Zl> {

	private  User userObj;
	private Map<String, Object> sessionMap;
	private Zl graduate = new Zl();
	...
	public PatentServiceI graduateService;
	public PatentServiceI getPatentService() {
		return graduateService;
	}

	@Autowired
	public void setPatentService(PatentServiceI graduateService) {
	...
	public void add() {
		...
		try {
			...
			Zl r = graduateService.save(graduate);
			j.setSuccess(true);
			j.setMsg("添加成功！");
			j.setObj(r);
	...
	public void edit(){
		...
		try {
			Zl r = graduateService.edit(graduate);
			j.setSuccess(true);
			j.setMsg("修改成功！");
			j.setObj(r);
		}catch(DataIntegrityViolationException e){
